...
export function MeasureRow({ measure, onEdit, onDelete }: MeasureRowProps) {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editedQuantity, setEditedQuantity] = useState(measure.quantity);

  const [editedMeasure, setEditedMeasure] = useState<any>({
    value: measure.id,
  ...
  return (
    <>
      <TableRow>
        <TableCell className="hidden sm:table-cell">{measure.id}</TableCell>
        <TableCell className="font-medium">{measure.quantity}</TableCell>
        <TableCell className="font-medium">
          {measure?.device?.deviceName!}
        </TableCell>
        ...
        <TableCell className="font-medium">
          {measure.updatedAt
            ? new Date(measure.updatedAt).toLocaleDateString()
            : "-"}
        </TableCell>
        ...
        <TableCell>
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem onClick={() => onDelete(measure.id)}>
                Delete
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
