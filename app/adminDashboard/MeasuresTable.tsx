...
import { getMeasures, addMeasure, deleteMeasure as deleteMeasureAPI, updateMeasure } from "@/app/adminDashboard/api/apiMeasure";
...
const MeasuresTable: React.FC = () => {
  ...
  const [currentMeasure, setCurrentMeasure] = useState<Partial<Measure> | null>(null); // For editing a measure
  ...
  const handleUpdateMeasure = async (id: number, updatedMeasure: { quantity: number }) => {
    ...
    try {
      ...
      setMeasures((prev) =>
        prev.map((measure) => (measure.id === id ? { ...measure, ...updated } : measure))
      );
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-card p-5 rounded-lg shadow-lg max-w-sm w-full z-50">
            <h2 className="text-xl font-bold mb-4 text-primary-foreground">Add New Measure</h2>
            <input
              ...
              onChange={(e) => setNewMeasure({ ...newMeasure, quantity: parseInt(e.target.value) })}
            />
